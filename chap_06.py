...
class FakeDatabase:
    ...
    def update_user_info(self, key, value, new_email=None, new_phone=None):
        if key not in {"email", "phone", "username"}:
        ...
        for customer in self.customers:
            if customer[key] == value:
                ...
                return f"Updated user info for {customer['name']}"
        return f"Couldn't find a user with {key} of {value}"
...
tools = [
    ...
    },
    {
        ...
        "description": "Updates a user's email or phone number using their email, phone, or username as identifier.",
        "input_schema": {
            "type": "object",
            "properties": {
                "key": {
                    "type": "string",
                    "enum": ["email", "phone", "username"],
                    "description": "The user identifier type."
                },
                "value": {
                    "type": "string",
                    "description": "The current value of the identifier (e.g., current email or phone or username)."
                },
                "new_email": {
                    "type": "string",
                ...
                "new_phone": {
                    "type": "string",
                    "description": "The new phone number. Optional."
                }
            },
            "required": ["key", "value"]
        }
    ...
    {
        "name": "get_user_info",
        "description": "Retrieves both user info and their order history given email, phone, or username.",
        "input_schema": {
            "type": "object",
            "properties": {
                "key": {
                    "type": "string",
                    "enum": ["email", "phone", "username"],
                    "description": "Search field for user."
                },
                "value": {
                    "type": "string",
                    "description": "Search value for the user field."
                }
            },
            "required": ["key", "value"]
        }
...
def process_tool_call(tool_name, tool_input):
    if tool_name == "get_user":
        ...
        return db.update_user_info(
            tool_input["key"],
            tool_input["value"],
            tool_input.get("new_email"),
            tool_input.get("new_phone")
        )
    elif tool_name == "get_user_info":
        user = db.get_user(tool_input["key"], tool_input["value"])
